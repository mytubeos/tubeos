...
export const useChannel = () => {
  ...
  const handleOAuthReturn = useCallback(async (searchParams) => {
    ...
    if (connected === 'true') {
      toast.success(channel ? `"${channel}" connected successfully!` : 'YouTube channel connected!')
      await fetchChannels()
    ...
    if (error) {
      const msgs = {
        access_denied:     'You cancelled the YouTube connection.',
        missing_params:    'Something went wrong. Please try again.',
        no_refresh_token:  'Could not get full access. Remove this app from myaccount.google.com/permissions and try again.',
