...
import { TrendingUp, Plus, Target, Flame } from 'lucide-react'
...
export const Growth = () => {
  ...
  const fetchAll = async () => {
    ...
    setLoading(true)
    try {
      const promises = [analyticsApi.getGrowth(channelId), analyticsApi.getSuggestions(channelId)]
      ...
      if (results[1].status === 'fulfilled')
        setSuggestions(results[1].value.data.data?.suggestions || [])
      if (canAccessCompetitors && results[2]?.status === 'fulfilled')
        setCompetitors(results[2].value.data.data || [])
      if (canAccessTrends && results[3]?.status === 'fulfilled')
        setTrends(results[3].value.data.data?.trends || [])
    } catch {
  ...
  const handleAddCompetitor = async () => {
    if (!competitorUrl.trim()) {
      toast.error('Enter a channel ID or URL')
      return
    }
      ...
      <Card>
        ...
        {!canAccessCompetitors ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 mb-3">Track competitors and steal their strategy</p>
        ...
        {!canAccessTrends ? (
          ...
          <div className="space-y-2">
            {trends.map((trend, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-3 glass rounded-xl hover:border-white/12 transition-all"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    {trend.category && (
                      <span className="text-2xs text-gray-500">{trend.category}</span>
                    )}
                    {trend.growthRate && (
                ...
                <div className="text-right shrink-0">
                  <p
                    className={`text-sm font-bold text-${opportunityColor(trend.opportunityScore)}`}
                  >
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button size="sm" onClick={handleAddCompetitor} loading={addingCompetitor}>
              Add
            </Button>
          </>
        }
