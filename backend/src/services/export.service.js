 ...
 * @returns {string}
 */
const buildCsv = (rows, channelName, period, brand = {}) => {
  const totals = summarise(rows);
  const lines = [];

  // Metadata header
  lines.push(`# ${brand.companyName || 'Vezrin'} Analytics Report`);
 ...
 */
const buildPdf = (rows, channelName, period, brand = {}) => {
  return new Promise((resolve, reject) => {
    ...
    // ---- Header bar ----
    doc.rect(50, 30, W, 70).fill(brand.color || BRAND);
    doc
      .fillColor('white')
      .font('Helvetica-Bold')
      .fontSize(20)
      .text(`${brand.companyName || 'Vezrin'} Analytics Report`, 65, 45);
    doc
      .fillColor('#C7D2FE')
      .font('Helvetica')
      .fontSize(10)
    ...
    doc
      ...
      .text(
        brand.companyName
          ? `Generated by ${brand.companyName}`
          : 'Generated by Vezrin • tubeos.saas@gmail.com',
        50,
        footerY,
 ...
 * @param {object} [brand] — { companyName, color }, from getReportBrand(user)
 * @returns {Promise<Buffer>}
 */
const buildReportPdf = (reportData, user, brand = {}) => {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 50, size: 'A4' });
    const chunks = [];
    doc.on('data', (c) => chunks.push(c));
    ...
    // ---- Header ----
    doc.rect(50, 30, W, 75).fill(brand.color || '#04263D');
    doc
      .fillColor('white')
      .font('Helvetica-Bold')
      .fontSize(22)
    ...
    doc
      ...
      .fontSize(8)
      .text(
        `Generated by ${brand.companyName || 'Vezrin'}  •  ${new Date().toDateString()}  •  ${user?.email || ''}`,
        50,
        footerY,
        { align: 'center', width: W }
      );
...
/**
 * Whether/what a user's reports should be white-labeled with. Re-checks
 * plan at render time (not just at PATCH /auth/branding write time) so a
 * later downgrade can't leave a stale white-labeled report going out.
 * @param {object} user — needs at least { plan, branding }
 * @returns {{companyName?: string, color?: string}} empty object if not entitled/enabled
 */
const getReportBrand = (user) => {
  if (user?.plan !== 'agency' || !user?.branding?.enabled) return {};
  const brand = {};
  if (user.branding.companyName) brand.companyName = user.branding.companyName;
  if (user.branding.primaryColor) brand.color = user.branding.primaryColor;
  return brand;
};
