...
const gatherReportData = async (userId, channelId) => {
  ...
  const [channel, thisWeek, lastWeek, dailyRows, topVideos] = await Promise.all([
    YoutubeChannel.findOne({ _id: channelId, userId, isActive: true })
      .select('channelName thumbnail stats'),
    ...
    ChannelAnalytics.find({ channelId: toObjId(channelId), date: { $gte: weekAgo } })
      .sort({ date: 1 }).select('date metrics.views').lean(),

    Video.find({ userId, channelId, status: 'published' })
      .sort({ 'performance.views': -1 }).limit(3)
      .select('title thumbnail youtubeVideoId publishedAt performance isShort').lean(),
  ]);
  ...
  if (!curr.views) {
    const vAgg = await Video.aggregate([
      ...
      { $group: { _id: null, views: { $sum: '$performance.views' }, likes: { $sum: '$performance.likes' }, comments: { $sum: '$performance.comments' } } },
    ]);
    if (vAgg[0]) {
  ...
  const kpis = {
    ...
    watchTime:  { value: Math.round((curr.watchTime || 0) / 60), change: pctChange(curr.watchTime, prev.watchTime) },
    subscribers:{ gained: curr.subsGained || 0, lost: curr.subsLost || 0, change: pctChange(curr.subsGained, prev.subsGained) },
    likes:      { value: curr.likes      || 0 },
    comments:   { value: curr.comments   || 0 },
    ctr:        { value: parseFloat((curr.avgCtr || 0).toFixed(2)), change: pctChange(curr.avgCtr, prev.avgCtr) },
  };
  ...
  return {
    channel:    { name: channel.channelName, thumbnail: channel.thumbnail, subscribers: channel.stats?.subscriberCount || 0 },
    kpis,
    dailyViews,
    bestDayName,
    topVideos,
...
const generateInsights = ({ kpis, prev, topVideos, bestDayName, channel }) => {
  ...
  if (topVideos.length >= 2) {
    ...
    if (keywords) {
      ...
      const avgOthers = topVideos.slice(1).reduce((s, v) => s + (v.performance?.views || 0), 0) / Math.max(topVideos.length - 1, 1);
...
const generateActionItems = ({ insights, kpis, topVideos }) => {
  ...
  // From top video
  if (topVideos[0]) {
    items.push(`Reply to comments on "${topVideos[0].title?.slice(0, 35)}..." — engagement in first 24h boosts reach`);
  }

  // From insights
  const warning = insights.find(i => i.color === 'warning');
  if (warning) {
    items.push(warning.type === 'warning' && warning.icon === 'alert-triangle'
...
const calcMilestones = (current, weeklyGain) => {
  ...
  return next.map(t => ({
    ...
    sublabel:  t === 1000 ? 'Full monetization eligible' : t === 100 ? 'Community posts unlock' : 'YouTube milestone',
...
const buildDailyArray = (rows, weekAgo) => {
  const map = {};
  rows.forEach(r => { map[r.date.toISOString().split('T')[0]] = r.metrics?.views || 0; });
...
const extractKeyword = (title) => {
  ...
  const stop  = new Set(['this','that','with','your','from','have','been','will','they','what','when','were','their','there']);
